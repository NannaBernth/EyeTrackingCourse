...
setwd("C:/Users/nanna/OneDrive - Aarhus universitet/4. Semester/Social And Cultural/Day 5")
...
ggplot(Fixations2018, aes(x=StartTimeS, y=PupilSize, color=ParticipantGender)) + 
  geom_smooth(method = "auto", se = F) + facet_grid(Position~Gender) +
  labs(title ='Distribution of Duration of Fixations Acording to Task')

ggplot(Fixations2018, aes(x=Fixation, y=PupilSize, color=ParticipantGender)) + 
  geom_smooth(method = "auto", se = F) + facet_grid(Position~Gender) +
  labs(title ='Distribution of Duration of Fixations Acording to Task')
...
pup_1 <- glmer(PupilSize ~ Ostension * Position + (1 | ParticipantID), family = gaussian(link = log), FixationsSE, REML = F)

pup_2 <- glmer(PupilSize ~ Ostension * Position + StartTimeS + (1 | ParticipantID), family = gaussian(link = log), FixationsSE, REML = F)
...
pup_5 <- glmer(PupilSize ~ ParticipantGender * Gender + (1 | ParticipantID), family = gaussian(link = log), FixationsSE, REML = F)

pup_6 <- glmer(PupilSize ~ Position + ParticipantGender * Gender + (1 | ParticipantID), family = gaussian(link = log), FixationsSE, REML = F)

pup_7 <- glmer(PupilSize ~ Ostension + Position + (1 | ParticipantID), family = gaussian(link = log), FixationsSE, REML = F)
